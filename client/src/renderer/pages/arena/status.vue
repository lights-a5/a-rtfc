<template>
  <v-container fluid fill-height>
    <v-layout column justify-space-around v-if="$store.state.loggedIn" class="status">
      ...
      <v-flex xs1 :style="s.extinstalled">
        <v-layout row align-center>
          <div :class="{'grey--text': !$store.state.enabledFeatures.extensionManagement}">A:RTFC extension installed</div>
          ...
          <template v-if="$store.state.enabledFeatures.extensionManagement && $store.state.status.extinstalled === false">
            <v-btn small color="" @click="open('<<<<<extension_url>>>>>')">INSTALL</v-btn>
          </template>
        </v-layout>
      </v-flex>
      <v-flex xs1 :style="s.extactive">
        <v-layout row align-center>
          <div :class="{'grey--text': !$store.state.enabledFeatures.extensionManagement}">A:RTFC extension active</div>
          <template v-if="!$store.state.enabledFeatures.extensionManagement">
            <v-spacer />
            <v-btn small color="" @click="open($store.getters.loginURL(true))">LOGIN</v-btn>
            <div class="body-1">to unlock this feature</div>
      ...
      <v-flex xs1 :style="s.deckmasterws">Connected to A:RTFC server</v-flex>
    </v-layout>
    <v-layout column justify-center align-center v-else class="login">
      ...
      <p>
        For A:RTFC to operate we require you login to verify your identity. <br>
        ...
        - Adjusting your extensions allows us to enable &amp; disable A:RTFC automatically <br>
      ...
      <p v-if="$store.state.token">
        We've detected existing authorization from an old version of A:RTFC. If you'd like to continue without the features described above, <a href="#" @click="$store.commit('fakeLogin')">click here</a>.
      </p>
    </v-layout>
  </v-container>
</template>
