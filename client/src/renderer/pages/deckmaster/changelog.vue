...
export default {
  ...
  created() {
    ghpromise.then(d => {
      let tempReleases = [{
        name: 'v202002.22.0',
        changes: ["Changed to A:RTFC. Will now use server hosted by Jeff Hoogland.", 
        "Added Linux Support. Set ARENA_LOG_PATH", 
        "Removed Streamdecker Tab"]
      }]
      let oldReleases = d.map(r => {
        return {
          name: r.tag_name,
          changes: r.body
            .trim()
      ...
      tempReleases.push(...oldReleases)
      this.releases = tempReleases
    })
  },
}
</script>
